...
async def create_new_product(
    ...
    return await crud_product.create_product(product_data=product_data, session=session)

@router.get("/", response_model=List[ProductPublic])
async def get_all_products_list(
    session: AsyncSession = Depends(get_session)
):
    return await crud_product.get_all_products(session=session)
...
async def get_product_details(
    ...
    if not product:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Product with ID {product_id} is not available")
    return product